...
\begin{definition}
    Uma função \(f: (X,\tau_X) \to (Y, \tau_Y) \) é contínua num ponto \(a \in X\) se \(\forall V\ab \subset Y : f(a) \in V\), \(\exists U\ab\subset X : a \in U\) e \(f(U)\subset V\).
\end{definition}
...
\begin{proposition}
    \textbf{8.3.} Dada \(f : (X, \tau_X) \to (Y, \tau_Y)\) são equivalentes:
    \vspace{-0.3cm}
...
\begin{definition}
    Diz-se que uma familia de funções \((f_i: X \to X_i)_{i\in I}\) \emph{separa pontos} se \(\forall x, y \in X : x \neq y, \ \exists j \in I : f_j(x) \neq f_j(y)\). 
\end{definition}

\hypertarget{prop108}{}
\begin{proposition}
    \textbf{10.8.} Sejam \(f_i : (X,\tau) \to (X_i, \tau_i)\) e \(\epsilon_{_X} : X \ni x \mapsto (f_i(x))_{i\in I} \in \prod X_i\) (\emph{avaliação}). Então, \(\epsilon_{_X}: X \simeq \operatorname{Im}\epsilon_{_X} \leq \prod X_i\) se e somente se verificar: 
    \[\text{(E1) \ A familia }(f_i)_{i\in I} \text{ separa pontos} \hspace{0.8cm}  \text{e \hspace{0.8cm}(E2) \ }\tau = \tau_w \text{ definida pelas } (f_i). \]
\end{proposition}
\comentario{Não suporta resumo, olha se quiser \cite[pag. 27]{mujica2005}}

\subsection*{O espaço quociente}

\begin{proposition}
    \textbf{11.1.} Sejam \((X, \tau)\), \(Y\) um conjunto qualquer e \(q : X \twoheadrightarrow Y\). Então, a coleção 
    \[\tau_q := \{V\subset Y : q^{-1}(V)\ab \subset X\}\] 
    é uma topologia em \(Y\) que chamamos de \emph{topologia quociente} dada por \(q\).
\end{proposition}

\begin{definition}
    Uma aplicação \(q: (X,\tau_X) \twoheadrightarrow (Y, \tau_Y)\) é \emph{quociente} se \(\tau_Y = \tau_q\).  
\end{definition}

\begin{proposition}
    \textbf{11.3.} A topologia \(\tau_q\) é a mais fraca tal que \(q: X \to (Y,\tau_q)\) é contínua. 
\end{proposition}

\begin{proposition}
    \textbf{11.4.} Sejam \(q : X \to Y\) aplicação quociente e \(g: Y \to (Z,\tau_Z)\) uma função. Então, \(g\) é contínua \ \(\leftrightharpoons\) \ \(g\circ q: X \to Z\) é contínua.
\end{proposition}

\hypertarget{prop115}{}
\begin{proposition}
    \textbf{11.5.} Seja \(q: X \twoheadrightarrow Y\) contínua. Se \(q\) é aberta (ou fechada), então \(\tau_Y = \tau_q\). \comentario{É coisa de provar as duas contenções, não é díficil}
\end{proposition}

\begin{example}
    Seja \(q : [0,2\pi] \to \E^1\) tal que  \(t \mapsto (\cos t, \sin t)\). \comentario{Sobrejetiva e fechada}
\end{example}
...
\begin{proposition}
    \textbf{11.9.} Dada \(q\) aplicação quociente, existem \(P\) aplicação decomposição e \(f: Y \simeq \mathcal{D}  \) tal que \(f \circ q = P\). \comentario{Olha \cite[pag. 30]{mujica2005}}
\end{proposition}
