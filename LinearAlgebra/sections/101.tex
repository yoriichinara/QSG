...
\begin{definition}
    Sejam \(P_1, \ P_2\) propriedades de funções. Dados \(X, \ U\) conjuntos e \(\phi \in \mathcal{F}(X,U)\), diz-se que \((\phi, U)\) é \emph{universal} em \(X\) respeito de \(P_1\) e \(P_2\) se: 
...
\begin{definition}
    Uma propriedade funcional \(P\) é \emph{compatível com composições} se \(f \circ g \mid P\) toda vez que \(\exists f \circ g\) e ambas \(f\text{ e } g\) verifiquem \( P \). 
\end{definition}
