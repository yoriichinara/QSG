...
\begin{definition}
    Sejam \(f:U\ab\subseteq \R \to \R\) e \(p\in U\). Dizemos que \(f \) é \emph{diff} em \(p\) sse \(\exists \ell \in (\R^n)'\) tal que \(f(p+v) = f(p) + \ell\cdot v + \sigma\|v\|, \text{ quando} \ v\to 0. \footnote{\((\R^n)' := \hom_\R(\R^n, \R)\), o espaço dual, olha as notas de Álgebra Linear II.}\)
\end{definition}
%\vspace{-0.4cm}
\begin{note}
...
\begin{definition}
    ...
    \(df(p)\cdot v := \sum^n \frac{\partial f}{\partial x_j}(p)\cdot v_j\).
