...
\begin{definition}[Posto]
  Se \(L\in \mathcal{L}(\R^n ;\R^m)\), então \(\pt L:= \dim [Le_i]\leq \min \{n,m\}\).  
  \[\pt L = \max\{ r: \exists M_r(L)\text{ com } \det (M_r)\neq 0\}.\]
  \[\pt L = \min\{n, m\} \Leftrightarrow \begin{cases}
...
\begin{note}
    A função \(\pt: U \to \N\) é semi-continua inferiormente.
\end{note}
