...
\begin{example}
    \(^{\infty}_{\hspace{0.15cm}n} U\ab,\ ^{\hspace{0.3cm}\infty}_{n-1}\E^{n-1} \subseteq \R^n\). \textcolor{gray}{\(\rightarrow \) \(\varphi = \id_n\) e projeção estereográfica}
\end{example}

\begin{lemma}
    Sejam \(^k_d M \subseteq \R^n \) e \(\psi: V\ab\subseteq \R^d \to M\subseteq \R^n\). Então, \(\psi\) é paremetrização local da \(M\) \(\leftrightharpoons\) \(\psi \) é imersão e homeomorfismo (\(\tau_{_M}\)). 
\end{lemma}
...
\begin{example}[\emph{Coordenadas Esféricas}]
    ...
    \[\textstyle (\theta_i) \mapsto \left(\cos\theta_1, \ldots, \prod^{j<i}\sin\theta_j \cdot \cos\theta_{i},\ldots, \prod\sin\theta_i\right). \] 
\end{example}
...
\begin{definition}
    ...
    \[ T_pM := \operatorname{Im}(D\psi_a(a))= \bigoplus \R \frac{\partial \psi_a}{\partial x_i} (a)\leq \R^n.\  \footnote{O \emph{espaço afim} \(p + T_pM\) é o "visualmente" tangente a \(M\) em \(p\). }\]
\end{definition}
...
\begin{note}
    De aquí pra frente \(M \sim \ ^{\hspace{0.1cm}k}_{d_1}M\subseteq \R^n\) e \(N \sim \ ^{\hspace{0.1cm}k}_{d_2}N\subseteq \R^m\).
\end{note}
...
\begin{note}
    ...
    \begin{description}
        ...
        \item[Teorema da Função Inversa] Seja \(F\in C^k(M,N)\) tal que \(D\widetilde{F}(p)\) é isomorfismo, então \(\exists \ ^k F : \U_p \simeq \mathcal{U}_{F(p)}\).  
    \end{description}
\end{note}
