...
\[(\textbf{x}, x_j) \mapsto F(\textbf{x}) + \sum x_jv_j.\] Use \(\blacksquare\) \hyperlink{thmTFI}{(\emph{Função Inversa})} em \(H\) no \((p,0)\). Finalmente, faça \(G = H^{-1}\) e estableça as vizinhanzas.\vspace{0.5cm}} \parbox[b]{0.3\linewidth}{  }
} 
...
\end{theorem}
\demo{\parbox[b]{0.35\linewidth}{ \textcolor{white}{dx} }\parbox[a]{0.65\linewidth}{\vspace{0.6cm}Tire uma base l.i. \([DF(p)\cdot e_j]= \R^m\). Tome \(H: U \to \R^m \times \R^{n-m}\) tal que \(\textbf{x}\mapsto (F(\textbf{x}), x_j)\). Estude \([DH(p)\cdot v]\), segue do \(\blacksquare\) \hyperlink{thmTFI}{(\emph{Função Inversa})} que \(H\) é diffeomorfismo. Faça \(G= H^{-1}\), estableça as vizinhanzas e conclua.\vspace{0.6cm}}  
}
...
\begin{note}
  \(\pt L = \min\{n,m\}\) \(\leftrightharpoons\) \(L\) é injetiva, sobrejetiva ou isomorfismo. Também, vista como função \(\pt: U \to \N\) é semi-continua inferiormente.
\end{note}
...
\begin{corollary}%[Colorarios]
  ...
  \begin{enumerate}[label = \roman*.]
    ...
    \item Se \(F\) é localmente injetiva, então é imersão em \(V\ab\subset U\) denso em \(U\). \textcolor{gray}{\(\rightarrow \) Segue do \(\blacksquare\) \hyperlink{thmPC}{(\emph{Posto Constante})} e da semi-continuidade inferior}
    \item Se \(F\) é aberta, então é submersão em \(V\ab\subset U\) denso em \(U\). 
  \end{enumerate}
\end{corollary}

\alerta{
  \centering O posto deve ser constante em TODA uma vizinhanza
  \tcblower
\begin{example}
  \(F: \R^2 \ni (t,s)\mapsto (t^2, t^3, s ) \in \R^3\) ¿Existem coordenadas como no \(\blacksquare\) \hyperlink{thmPC}{(\emph{Posto Constante})} nalguma vizinhança do origem? \textcolor{gray}{\(\rightarrow \) Não} %\(\operatorname{rank}(JF(0,0))\)}
\end{example}
}