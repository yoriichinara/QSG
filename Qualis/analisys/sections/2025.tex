...
\begin{align*}
\lim_{\|(h,k)\| \to 0} \frac{B(h,k)}{\|(h,k)\|} &\leq \lim_{\|(h,k)\| \to 0} \frac{C\|h\|_1\|k\|_1}{\|(h,k)\|}\\ 
&\leq \lim_{\|(h,k)\| \to 0} C\|k\|_1 = 0.  
\end{align*} 
Logo, \(B\) é diff e \(DB(\x,\y)(h,k) = B(\x, k) + B(h,\y)\), linear em \((h,k)\) de fato. }  \\ 
...
\end{align*}
Note que \(G\) é linear quanto \(\mu\) é bilinear, do \(\blacksquare\) (\emph{RC}) e ítem (b), 
\(DF(X)(H) = D\mu(G(X))DG(X)(H) = D\mu(X,X^t)(H,H^t) = XH^t + HX^t\). Sejam \(X \in M_n(\R) : F(X) = XX^t = \id_n\) e \(S \in \operatorname{Sym}_n(\R)\) e \(H:= \nicefrac{1}{2}SX\),   
\[DF(X)(H) = \nicefrac{1}{2}X(SX)^t + \nicefrac{1}{2}(SX)X^t = S. \]
Portanto \(DF\) é sobrejetiva e \(\id_n\) valor regular. 
}  \\
...
(b) Se \(M\) é uma variedade com bordo e \(M \) é orientável, explique o que significa \(M\) ser orientável e como é definida a orientação em \(\partial M\). 
