...
\begin{lemma}
    \begin{enumerate}[label = \roman*., left = -0.3cm]
        ...
        \item[]\hspace{-0.23cm}ii. \ Se \(V = V^\perp \oplus W\), então \(\rad W = \{0\}\). \comentario{\(\rad W \subseteq V^\perp \)}  
    \end{enumerate}
\end{lemma}
...
\parbox[a]{0.77\linewidth}{
    \begin{note}
        Se \(V\) fora hiperbólico, então na base (ordenada) \(\alpha = (v_j, w_j)\) temos \([\phi]_\alpha = H_m= \operatorname{diag}(H_j)\), \(j\leq m\). Em particular, \(\phi\) é não degenerada.
    \end{note}
