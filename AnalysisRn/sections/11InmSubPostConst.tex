...
\begin{corollary}%[Colorarios]
  Em geral temos, \(\blacksquare\) \hyperlink{thmTFI}{(\emph{Função Inversa})} \(\leftrightharpoons\) \(\blacksquare\) \hyperlink{thmFLI}{(\emph{FLI})} e \(\blacksquare\) \hyperlink{thmFLS}{(\emph{FLS})} \(\leftrightharpoons\) \(\blacksquare\) \hyperlink{thmPC}{(\emph{Posto Constante})}. Sejam \(F\in C^1(U,\R^m)\) e \(p\in U\). Então, 
