...
\begin{definition}
    Sejam \(f:U\ab\subseteq \R \to \R\) e \(p\in U\). Dizemos que \(f \) é \emph{diff} em \(p\) sse \(\exists \ell \in \hom(\R^n,\R)\) tal que \(f(p+v) = f(p) + \ell\cdot v + \sigma(\|v\|), \text{ \ quando} \ v\to 0. \footnote{\(\hom(\R^n, \R)= \hom_\R(\R^n, \R)\), olha as notas de Álgebra Linear II.}\)
\end{definition}
%\vspace{-0.4cm}
\begin{note}
