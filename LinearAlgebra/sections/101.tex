...
\begin{lemma}
    \textbf{10.1.3.} Sejam \((\phi_a, U)\) e \((\phi_b, V)\) universais em \(X\) respeito de \(P_1\) e \(P_2\). Se \(\phi_a, \  \phi_b \mid P_1\); \ \(\id_{_U}, \ \id_{_V} \mid P_2\) e \(P_2\) é compatível com composições, então \(\exists ! f : U\to V\mid P_2\) tal que \(\phi_b = f \circ \phi_a\). Além disso, aquela \(f\) é bijeção. 
\end{lemma}
