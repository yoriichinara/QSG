...
\begin{definition}
    Uma aplicação \(F\) diff em \(p\) é \emph{imersão} em \(p\) se \(DF(p): \R^n \to \R^m\) fora injetora. Em particular, \(n\leq m\).
\end{definition}
...
\[(\textbf{x}, x_j) \mapsto F(\textbf{x}) + \sum x_jv_j.\] Use \(\blacksquare\) \hyperlink{thmTFI}{(\emph{Função Inversa})} em \(H\) no \((p,0)\). Finalmente, faça \(G = H^{-1}\) e estableça as vizinhanzas.\vspace{0.2cm}} \parbox[b]{0.3\linewidth}{  }
} 
...
\begin{definition}
    Uma aplicação \(F\) diff em \(p\) é \emph{submersão} se \(DF(p):\R^n\to \R^m \) fora sobrejetora. Em particular, \( n\geq m\).  
\end{definition}
...
\begin{theorem}[\emph{\textbf{F}orma \textbf{L}ocal das \textbf{S}ubmersões}]
   ...
   \[F\circ G : V \ni (\textbf{x}, x_{m+1}, \ldots, x_n) \mapsto \textbf{x} \in \R^m.   \]
\end{theorem}
\demo{\parbox[b]{0.35\linewidth}{ \textcolor{white}{dx} }\parbox[a]{0.65\linewidth}{\vspace{0.5cm}Tire uma base l.i. \([DF(p)\cdot e_j]= \R^m\). Tome \(H: U \to \R^m \times \R^{n-m}\) tal que \(\textbf{x}\mapsto (F(\textbf{x}), x_j)\). Estude \([DH(p)\cdot v]\), segue do \(\blacksquare\) \hyperlink{thmTFI}{(\emph{Função Inversa})} que \(H\) é diffeomorfismo. Faça \(G= H^{-1}\) e estableça as vizinhanzas.\vspace{0.5cm}}  
}
...
\begin{definition}[\emph{Posto}]
  Se \(L\in \hom(\R^n, \R^m)\) então \(\pt L:= \dim( \operatorname{Im} L)\leq \min \{n,m\}\) equivalente a \( \max\{ r: \exists M_r(L)\text{ com } \det M_r\neq 0\}\). 
...
\begin{theorem}[\emph{Posto Constante}]
    Seja \(F \in C^k(\U_p, \R^m)\) tal que \(\forall \x\in \mathcal{U}_p\), \(\pt DF(\textbf{x}) = r\). Então, \(\exists  ^k G: V\ab \to \mathcal{V}_p\) e \(\exists ^k H : \mathcal{W}_{F(p)} \to W\ab\) tais que, 
...
\item[] \begin{tcolorbox}[colframe=gray, colback=white, boxrule = 0.8pt] 
  ...
  \tcblower 
\parbox[a]{0.85\linewidth}{\textcolor{gray}{Note que, sendo \(q = G^{-1}(p)\), \(\exists \mathcal{V}_q= V\) onde \(\pt J(F\circ G)(\textbf{x})=r\), ou seja que \(D\equiv 0\). Logo, \(y_i(\textbf{x}) = y_i(\textbf{x}^{(r)}, q^{(n-r)})\). 
}\vspace{-0cm}}
...
\end{tikzpicture}
\\ 
\vspace{0.3cm}Figura 1: \(\blacksquare\) \hyperlink{thmPC}{(\emph{Posto Constante})}  
\end{figure}
...
\begin{corollary}%[Colorarios]
  ...
  \begin{enumerate}[label = \roman*.]
    \item Se \(\pt JF(p) = r=\min \{n,m\}\), então \( \exists \mathcal{V}_p\) onde \(JF_r(\textbf{x}) \in GL_r(\R)\). 
...
\alerta{
  ...
  \tcblower
\begin{example}
  \(F: \R^2 \ni (t,s)\mapsto (t^2, t^3, s ) \in \R^3\) ¿Existem coordenadas como no Teorema em alguma vizinhanza do origem? \textcolor{gray}{\(\rightarrow \) Não} %\(\operatorname{rank}(JF(0,0))\)}
\end{example}
}