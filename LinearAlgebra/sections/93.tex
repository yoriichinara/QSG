\subsection{Pareamentos Bilineares}

Agora queremos um análogo geral do produto interno visto no \cite[Cap. 7]{MA719}, que aqui é apenas um exemplo de forma bilinear com \(\F \leq \R\). 
...
\begin{definition}
    Sejam \(V, W \) \(\F\)-espaços de dimensão finita, \(\alpha, \beta\) bases e \(\phi \in B(V,W)\). A \emph{matriz associada} \(_\alpha[\phi]_\beta := (\phi(v_i,w_j))_{i,j}\) determina \(\phi\) no sentido, \([v]_\alpha^t\  _\alpha[\phi]_\beta \  [w]_\beta = \phi (v,w)\). \footnote{Em geral, \(\Fi\) pode-se definir pra famílias de vetores. } 
\end{definition}
\begin{note}
...
\demo{Segue de propriedades das matrizes, pra a sobrejetividade pega \(A \in M_{m\times n}(\F)\) e mostra que \(\phi(v,w) := [v]_\alpha^t \cdot A \cdot [w]_\beta \in B(V,W)\). }
...
\begin{definition}
    ...
    \begin{itemize}[label = \(\star\)]
        ...
        \item \(\phi\) é \emph{não degenerada} a esquerda se \(\ker {_\phi D} = \{0\}\).\footnote{As definições pra o caso \emph{direita} são semelhantes.}
    \end{itemize}
\end{definition}
...
\begin{proposition}
    \label{prop:933}
    Se \(\alpha \) e \(\beta \) são bases finitas de \(V \) e \(W\) respectivamente, então \([ D_\phi]^\beta_{\alpha'} = \ _\alpha[\phi]_\beta\) e \([_\phi D]^\alpha_{\beta'} = ([ D_\phi]^\beta_{\alpha'})^t\). Em particular, \(\pt \phi := \pt D_\phi = \pt \hspace{0.01cm} _\phi D \). 
\end{proposition}

\demo{Como na \(\square\) \ref{prop:9.2.3} a gente ganha, de trabalhar com \(\alpha'\) mesma que a entrada \((i,j)\) de \([D_\phi]^\beta_{\alpha'}\) é justamente \(D_\phi(w_j,v_i) = \phi(v_i,w_j)\), a outra igualdade consegue-se num análise semelhante. }
...
\begin{corollary}
    ...
    \vspace{-0.2cm}
    \begin{enumerate}[left = -0.1cm, label = \roman*.]
        \item \(\{0\} = \ker D_\phi\);  \hfill ii. \(\dim V = \dim W\); \hfill iii. \(_\phi D : V \cong W'\); \hfill iv. \(D_\phi: W \cong V'\). 
    \end{enumerate}
\end{corollary}
...
\begin{definition}
    ...
    \begin{itemize}[label = \(\star\)]
        ...
        \- \hspace{3cm}\(\star\) \emph{Antissimétrica} \(\rightarrow \ \phi(v,w) =- \phi(w,v) \). 
    \end{itemize}
\end{definition}
...
\begin{proposition}
    ...
    \begin{enumerate}[label = (\alph*), left = 0.6cm]
        \item \(\phi \) é \textcolor{gray}{(antis)}simétrica \(\leftrightharpoons\) \(_\alpha [\phi]_\alpha\) é \textcolor{gray}{(anti)}simétrica.
        \item \(\phi \) é alternada \(\leftrightharpoons\) \(_\alpha [\phi]_\alpha\) é antissimétrica e \(\operatorname{diag}(_\alpha[\phi]_\alpha) = (0, 0, \ldots)\).% \ \leftrightharpoons \ \phi(v_i,v_i)= 0 \).
    \end{enumerate}
\end{proposition}
...
\begin{definition}
    \centering
    \(\alpha^{\perp_\phi} := \{w \in V:  \alpha \perp_\phi w\}\) \ e \  \(^{\perp_\phi}\alpha := \{v \in V: v \perp_\phi \alpha\}\). \footnote{Sendo \(\alpha = (v_i)\) entenda-se \(\alpha \perp_\phi w\) como os \(w \in V : \forall i \in I,\ v_i \perp_\phi w\).}
\end{definition}

\begin{note}
    Ambos \(\alpha^{\perp_\phi}\) e \(^{\perp_\phi}\alpha\) são os \emph{subespaços ortogonais} a direita e esquerda, respectivamente. Em particular, \(\ker D_\phi = V^{\perp_\phi}\) e \(\ker  {_\phi D} = \hspace{0.1cm}^{\perp_\phi}V\).
\end{note}
...
\begin{proposition}
    \label{prop:937}
    A relação \((V,\perp_\phi)\) é simétrica \(\leftrightharpoons\) \(\phi \) é simétrica ou alternada. \comentario{Não admite resumo, olha se quiser \cite[Pág. 309]{MA719}}
\end{proposition}
...
\alerta{
    ...
    \end{example}
    \begin{example}
        O espaço de Minkowski, \(V= \R^3 \times \R\) equipado com \(_\alpha [\phi]_\alpha = \operatorname{diag}(1,1,1,-1)\) é não degenerado mas qualquer \(w = (\textbf{x},\|\textbf{x}\|) \in V\) é isotrópico e, portanto \(\phi|_{_{W}}\) com \(W = [w]\) é degenerada em \(B(W)\).    
    \end{example}
}