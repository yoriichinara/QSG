...
\begin{theorem}[\emph{TVM - \(\R\)}]
  Seja \(\varphi: [0,1] \to \R\) função continua em \([0,1]\) e diff em \((0,1)\), então \(\exists \theta \in (0,1)\) tal que \(\varphi'(\theta) = \varphi(1) - \varphi(0)\). 
\end{theorem}
\begin{proposition}
...
\demo{Use o {\scriptsize\(\square\)} \hyperlink{lemma11}{1.1.}, aplique \(\blacksquare\) \hyperlink{thmTVMR}{\emph{(TVM - \(\R\))}} em \(\varphi(t) = f(p_i + te_j)\), quem é \(\varphi{'} (\theta)\)?}
