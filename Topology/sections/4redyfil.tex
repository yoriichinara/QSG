...
\begin{proposition}
    \textbf{13.7.} Sejam \(X := \prod X_i\) e \((\x_\lambda)_{\lambda \in \Lambda} \subset X\). Então, \(\x_\lambda \to \x \in X\) \ \(\leftrightharpoons\) \ \(\pi_i(\x_\lambda) \to \pi_i(\x) \in X_i, \ \forall i \in I\).
\end{proposition}
...
\begin{definition}
    Chamamos de \emph{subrede} de \(\x : \Lambda \to X\) qualquer rede da forma \(\x \circ \phi : M \to X\), onde \(M \) é dirigido e \(\phi:M \to \Lambda\) é uma função verificando:
    \vspace{-0.2cm}
