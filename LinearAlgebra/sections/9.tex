...
\begin{definition}
    Uma função \(\phi \in \mathcal{F}( \Pi^k V_j, W)\) é $k$\emph{-linear} se é linear em cada entrada. Ou seja, \(\forall j\leq k\), \(\forall (v_{i_0}) \in \Pi V_j \) fixo \(: v_{j_0} =0\), \(\forall(v_{i_1}), (v_{i_2}) \in \Pi V_j\) e \(\forall \lambda \in \F\), 
    \[  \textstyle \phi_j ((v_{i_0}) + (v_{i_1}+ \lambda v_{i_2})_{_{\delta_j}}) = \phi ((v_{i_0})+ (v_{i_1})_{_{\delta_j}}) + \lambda \phi((v_{i_0})+(v_{i_2})_{_{\delta_j}}).\footnote{Entendase \((v_i)_{_{\delta_j}}:= (v_i\cdot \delta_j)\).}\]  
\end{definition}
