...
\usepackage{times} % Fonte: Times New Roman
\usepackage{chngcntr} % Fonte: Times New Roman
...
{%{ \Large QUALI SURVIVAL GUIDE COLLECTION} \\ 
 \LARGE \textbf{\(^{+1}\)Notas de Análise em \(\mathbb{R}^n\)}} \vspace{0.1in}   \\
 { \today }\\ 
 {\small
...
{\small ** Conteúdo baseado na disciplina MM720 (Análise no R(n)) ministrada pelo professor Tiago Jardim Fonseca no período 2025-I. **\vspace{0.3cm}}%, destinado àqueles que já possuem alguma familiaridade com o assunto, pois não contém demonstrações formais, apenas as ideias por trás delas. Sinceramente, um fã da simplicidade. \\ 


%%%%%%%%%%%%%%%%%%%%%%%%% CONTENIDO %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
{ \textbf{Notação}}{\small  \hfill \text{\rotatebox{45}{\large $\boxdot$}}  Definição \hfill \(\textcolor{gray}{(-)}^{\text{\ding{71}}}\)  Aberto \hfill  \(\textcolor{gray}{(-)}^{\text{\ding{70}}}\) Fechado \\ 
{\scriptsize$\square$} \ Lema \hspace{2.5cm} {\large$\square$} \ Proposição \hspace{1.9cm}  {\large $\blacksquare$}  \ Teorema \hspace{2.3cm} {\scriptsize $\boxtimes$}  \ Corolário }
%\begin{center}
...
{\small 
\input{sections/1DirDerivatives.tex}
\input{sections/2CkFunctions.tex} 
\input{sections/3Diff.tex}
\input{sections/4MVT.tex}
\input{sections/5Taylor.tex}
\input{sections/6Optimization.tex}
\input{sections/7DiffAplications.tex}
\input{sections/8ChainsRule.tex}
\input{sections/9Diffeomorfismos.tex}
\input{sections/10IFT.tex} 
\input{sections/11InmSubPostConst.tex} 
\input{sections/12VarDiff.tex}
\input{sections/13AltForms.tex}
\input{sections/14IntVar.tex}
\input{sections/15LastOne.tex}
...
\bibitem{spivak2018}
Spivak, Michael (2018). \textit{Calculus on manifolds: a modern approach to classical theorems of advanced calculus}. CRC Press.
\end{thebibliography}
}
%\end{multicols}
