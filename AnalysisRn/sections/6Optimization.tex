...
\begin{example}
    ...
    \[\textstyle f(p+v)= f(p) + \cancel{df(p)} + \frac{1}{2}\sum^n \lambda_j v_j^2 + \sigma(\|v\|^2);\]
...
\begin{definition}
    Seja  \(g\in C^k(U)\) tal que \(\forall p \in \ker (g), \ dg(p)\neq 0\). O conjunto \(^k H:= \{\textbf{x} \in \R^n : g(\textbf{x}) = 0\}\) é uma \emph{hipersuperficie} de classe \(C^k\) definida por \(g(\textbf{x})=0\). 
\end{definition}
