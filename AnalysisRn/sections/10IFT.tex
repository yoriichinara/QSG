...
\item[] \begin{tcolorbox}[colframe=gray, colback=white, boxrule = 0.8pt]
    ...
    \tcblower 
    \textcolor{gray}{\(DR(0)=0\), logo do {\scriptsize \(\square\)} \hyperlink{lemma82}{8.2.}, com \(p=0\) e \(\epsilon = \frac{1}{2}\) sabemos \(\exists\V_0\) tal que \(\forall \textbf{x}_1, \textbf{x}_2 \in \V_0\), \(\|R(\textbf{x}_1) - R(\textbf{x}_2)\|\leq \frac{1}{2}\|\textbf{x}_1-\textbf{x}_2\|\). Leve aquela expressão a termos de \(F\) e concluia.}  
\end{tcolorbox}
\begin{definition}
...
\hypertarget{lemma83}{}
\begin{lemma}
    \textbf{8.3.} Seja \(L \in GL_n(\R)\). A aplicação \(\operatorname{inv}: L \mapsto L^{-1} \) é diff. \textcolor{gray}{\(\rightarrow \) Faz direito \(\text{inv}(L+H)\) e use a serie geométrica de Neumann}    
\end{lemma}
\item[] \begin{tcolorbox}[colframe=gray, colback=white, boxrule = 0.8pt]
    ...
    {\small \(^\bullet\)} A afirmação respeito da classe segue de {\scriptsize \(\square\)} \hyperlink{lemma83}{8.3.} no diagrama.
    }
\end{tcolorbox}
...
\hypertarget{prop85}{}
\begin{proposition}
    \textbf{8.5.} Se \(^k H\subseteq \R^n\) é hipersuperficie definida por \(g(\textbf{x})=0\), \(p\in H\) e \(c\in C^k(\mathcal{V}_\epsilon,H)\) é um caminho em \(H\), então \(\ker dg(p)= \{\dot{c}(0): c(0) = p \}\).
\end{proposition}

\demo{(\(\supseteq \)) Aplique \(\blacksquare\) \hyperlink{thmRDC}{(\emph{Regla da Cadeia})} na função \((g \circ c)(t)\). (\(\subseteq\)) \(\exists i \leq n \) tal que \(\frac{\partial g}{\partial x_i}(p) \neq 0\), suponha que é a última \(i=n\). Tome \(\vec{v}\in \R^n\) tal que \(dg(p)\cdot v=0\), aplique \(\blacksquare\) \hyperlink{thmTFIm}{(\emph{Função Implícita})} conseguindo \(c(t)= (p_j + tv_j, f(p_j+tv_j))\), finalmente, use nela o {\scriptsize \(\boxtimes\)} \hyperlink{corol83}{8.3.}. }
...
\begin{example}
    Para \(H = \mathbb{S}^{n-1}\) temos \(T_p H := \{\vec{v}\in \R^n : \langle p, v\rangle = 0\}\). \textcolor{gray}{\(\rightarrow\) Faça o exercício gráfico, note também que \((\nabla g(p))^t \perp T_pH(p)\)}
\end{example}

\hypertarget{lemma86}{}
\begin{lemma}
    \textbf{8.6.} Sejam \(V\) espaço vetorial sobre \(\R\) e \(\ell_1, \ell_2 \in V'\). Se \(\ker \ell_1 \subseteq \ker \ell_2\) então \(\exists \lambda \in \R \) tal que \(\ell_2 = \lambda \ell_1 \). 
\end{lemma}
\begin{corollary}
