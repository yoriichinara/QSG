...
\begin{lemma}
    \textbf{8.2.8.} Sejam \(f \in \Pol, v \in V, u = f(T)(v) \) e \(p = \mdc (f, m_v)\). Então, \(m_v = pm_u\). \comentario{Não suporta resumo, olha se queser \cite[Pág. 272]{MA719}}  
\end{lemma}
...
\begin{note}
    Em particular, se \(\gr (p_j) = 1\), então \(v_{sr+l} = v_s = (T - \lambda\id_{_V})^s(v_{i,j}), \ 1\leq s \leq k_{i,j}\), logo \(\mathscr{J}_{_T}(v) = \mathscr{C}_{p(T)}(v)\). Nesse chamamos tudo \textcolor{gray}{\rotatebox{90}{$\Big|$} \(\star\) \rotatebox{90}{$\Big|$}} \emph{de Jordan}.     
\end{note}
...
Portanto, \(V\) admíte base de Jordan, vamos estudar o fator \(V_{t-2}\),  
...
Agora, \(V_{t-2} = [e_2]\), \(\dim V_{t-2} = 1\), temos que seguir até conseguir \(\dim V_{(t-2)^{k}} = 3\), é conveniente façer conta da forma \((T-2\id)(w_{s}) = w_{s+1}\), então \vspace{1.8cm}
}
...
\] 
\text{\small Visto que não completamos a dimensão, contínuamos na siguente pôtencia, }
\[ \begin{bmatrix}
    2a \\ 
    b \\ 
    a \\
...
Por outro lado, visto que só temos \(1\) fator \(T\)-inv, \(V\) é \(T\)-cíclico, bastará tomar \(w = w_{1,2} + w_{1,0}\) e à base obvia do \(\mathscr{J}_{_T}(w) = \mathscr{C}_{T}(w) \), nesste casso da,
