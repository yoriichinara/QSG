...
\begin{itemize}[left=0cm]
    \item[] \vspace{-1cm} 
    \begin{exercise}
    \(\ C_T(v) = \{q(T)(v): q \in \Pol\}\). \comentario{Prova conjuntista}
    \end{exercise}  
    \vspace{-0.1cm}
    \item[] \demo{
    O resultado segue de mostrar \(\forall w \in C_T(v)\), \(C_T(v) = C_T(w)\). Prova as duas contenções usando o exercício acima e \(\blacksquare\) \hyperref[thm:bezout]{\hspace{0cm}(\emph{Bézout})}. 
}
\end{itemize}
...
\ejemplo{
    ...
    \begin{description}
        ...
        \item[Passo 2.] Encontre \(c_T= \det (t\id_{_V} - A)\) e sua fatoração em primos \(c_T = \prod^m p_j^{n_j}\). \comentario{Precisa habilidade na hora das contas do \(\det\)}
