...
2. Seja \(A \in M_n(\R)\subseteq M_n(\C)\). Sabemos \(\exists S \in \operatorname{GL}_n ( \C): S^{-1} A S = J =\) FCJ\((A)\). Mostre que os \(\lambda\)-blocos coincidem em longitude e comprimento com os \(\overline{\lambda}\)-blocos. \\ 
    ...
    \(\zeta \) está \underline{bem definida} toda vez que \(J\) e \(\overline{J}\) tem os mesmos blocos. Além disso, \(\zeta \) é \underline{bijeção}, pelo mesmo motivo. Assim, os \(\lambda\)-blocos coincidem como os \(\overline{\lambda}\)-blocos tanto em quantidade quanto comprimento. 
    } \vspace{0.1cm}\\
...
\hspace{-0.1cm}4. Enuncie e demostre o \(\blacksquare\) (\emph{Teorema Espectral}) para operadores hermitianos em \(\C\)-espaços de dimensão finita. \\ 
    \textcolor{gris}{
        \(\blacksquare\) (\emph{\(\C\)-Teorema Espectral}) Sejam \(V\) \(\C\)-espaço de dimensão finita com produto interno \(\langle\cdot \ ,  \cdot\rangle\) e \(T \in \ehom(V) : \langle v, T(w)\rangle = \langle T(v), w \rangle \). Então,
...
temos \([\phi]_{\alpha_\text{o}} = \operatorname{diag}([-\id_i], [\id_{p-i}])\). Supõe por contradição que \(\exists i \neq j: \phi(w_i, w_i) =1\) e \(\phi(w_j, w_j) = -1\), então \(w_i + w_j \neq 0 \) e, 
\[
