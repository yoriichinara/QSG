...
\begin{theorem}[\emph{Regla da Cadeia}]
    Sejam \(U\ab \subseteq \R^n\), \(V\ab \subseteq \R^r\), \(F: U\to \R^r\) diff em \(p\) tal que \(F(U)\subseteq V\) e \(G:V\to \R^m\) diff em \(F(p)\). Então \(G \circ F\) é diff em \(p\) e \(D(G\circ F)(p) = DG(F(p))\cdot DF(p)\). 
\end{theorem}
