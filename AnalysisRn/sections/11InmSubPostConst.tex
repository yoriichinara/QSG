...
\begin{definition}
    \(F\) diff em \(p\) é \emph{imersão} em \(p\) se \(DF(p): \R^n \hookrightarrow \R^m\). Em particular, \(n\leq m\).
\end{definition}
...
\ejemplo{
\centering \(c: I \subset \R \to \R^m\) caminho diff é imersão \textcolor{verdeoscuro}{\(\leftrightharpoons\)} \(\dot{c}(t) \neq 0\)
\tcblower
\begin{example}
...
\hypertarget{thmFLI}{}
\begin{theorem}[\emph{\textbf{F}orma \textbf{L}ocal das \textbf{I}mersões}] 
    Seja \(F \in C^k(U, \R^m)\) imersão em \(p\in U\). Então, \(\exists \mathcal{V}_p\subseteq U\) e \(\exists \ ^k G:\mathcal{W}_{F(p)} \simeq W\ab\) tais que \( G \circ F: \mathcal{V}_p \ni \textbf{x} \mapsto (\textbf{x}, 0) \in \R^m. \)
\end{theorem}
\demo{\parbox[a]{0.65\linewidth}{\vspace{0.4cm}Considere a base \([DF(p)\cdot e_i, v_j]= \R^m\). Tome \(H: U \times \R^{m-n} \to \R^m\) tal que, 
\[(\textbf{x}, x_j) \mapsto F(\textbf{x}) + \sum x_jv_j.\] Use \(\blacksquare\) \hyperlink{thmTFI}{(\emph{Função Inversa})} em \(H\) no \((p,0)\). Finalmente, faça \(G = H^{-1}\) e estabeleça as vizinhanças.\vspace{0.5cm}} \parbox[b]{0.3\linewidth}{  }
} 
\begin{tikzpicture}[scale= 0.9, >=to, shift={(13cm,5.1cm)}, baseline={(current bounding box.center)}, remember picture, overlay]%, shift={(14.85cm,-0.9cm)}, baseline={(current bounding box.center)}, remember picture, overlay]
  % Puntos
    \coordinate (O) at (1, -1.5); 
  % Espacio V
  \draw[<->, thick, color = gray, opacity = 0.5] (O) + (0,-1) -- + (0,1);
  \draw[thick] (O) + (0,-0.45) -- + (0,0.59);
...
\begin{definition}
  \(F\) diff em \(p\) é \emph{submersão} se \(DF(p):\R^n \twoheadrightarrow \R^m \). Em particular, \( n\geq m\).  
\end{definition}
...
\ejemplo{
\centering \(f: U\ab\subseteq \R^n \to \R\) diff em \(p\) é submersão \textcolor{verdeoscuro}{\(\leftrightharpoons\)} \(df(p)\neq 0\). 
\tcblower
\begin{example}
...
\hypertarget{thmFLS}{}
\begin{theorem}[\emph{\textbf{F}orma \textbf{L}ocal das \textbf{S}ubmersões}]
   Seja \(F\in C^k(U, \R^m)\) submersão em \(p\in U\). Então, \(\exists \mathcal{V}_p\subseteq U\) e \(\exists \ ^kG: V\ab \simeq  \mathcal{V}_p\) tais que \(F\circ G : V \ni (\textbf{x}, x_{m+1}, \ldots, x_n) \mapsto \textbf{x} \in \R^m   \).
\end{theorem}
\demo{\parbox[b]{0.35\linewidth}{ \textcolor{white}{dx} }\parbox[a]{0.65\linewidth}{\vspace{0.6cm}Tire uma base l.i. \([DF(p)\cdot e_j]= \R^m\). Tome \(H: U \to \R^m \times \R^{n-m}\) tal que \(\textbf{x}\mapsto (F(\textbf{x}), x_j)\). Estude \([DH(p)\cdot v]\), segue do \(\blacksquare\) \hyperlink{thmTFI}{(\emph{Função Inversa})} que \(H\) é diffeomorfismo. Faça \(G= H^{-1}\), estabeleça as vizinhanças e conclua.\vspace{0.6cm}}  
}

\- \vspace{-0.4cm}\begin{tikzpicture}[scale= 0.9, >=to, shift={(0.75cm,4.65cm)}, baseline={(current bounding box.center)}, remember picture, overlay]
  % Puntos
    \coordinate (W) at (0,-1); 
  % Espacio V
...
\begin{definition}[\emph{Posto}]
  Se \(L\in \hom(\R^n, \R^m)\), então \(\pt L:= \dim( \operatorname{Im} L)\leq \min \{n,m\}\). Também, se prefer,  \( \pt L = \max\{ r: \exists M_r(L)\text{ com } \det M_r\neq 0\}\). 
  %\[\textstyle \pt L = \min\{n, m\} \ \leftrightharpoons\  \begin{cases}
...
\begin{note}
  \(\pt L = \min\{n,m\}\) \(\leftrightharpoons\) \(L\) é injetiva, sobrejetiva ou isomorfismo. Também, vista como função \(\pt: U \to \N\) é semicontínua inferiormente.
\end{note}
...
\begin{theorem}[\emph{Posto Constante}]
    Seja \(F \in C^k(\U_p, \R^m)\) tal que \(\forall \x\in \mathcal{U}_p\), \(\pt DF(\textbf{x}) = r\). Então, \(\exists  ^k G: V\ab \simeq \mathcal{V}_p\) e \(\exists ^k H : \mathcal{W}_{F(p)} \simeq W\ab\) tais que, 
...
\tcblower
\textcolor{gray}{\([DF(p)]\cong \R^r\leq \R^m\). Aplique \(\blacksquare\) \hyperlink{thmFLS}{(\emph{FLS})} a \(\pi:(\x^{(r)},\x^{(m-r)}) \mapsto \textbf{x}^{(r)}\). Logo, \(\exists \ ^k G: V\ab\simeq \mathcal{V}_p\) tal que \(\pi\circ F\circ G:(\x^{(r)}, \x^{(n-r)}) \mapsto \x^{(r)}\).}
\end{tcolorbox}

\item[] \begin{tcolorbox}[colframe=gray, colback=white, boxrule = 0.8pt] 
  \centering \(F\circ G: \x^{(n)}\mapsto (\x^{(r)}, y_i(\x)^{(n-r)}) \)
  \tcblower 
\parbox[a]{0.83\linewidth}{\textcolor{gray}{\vspace{0.1cm}\\ Note que, sendo \(q = G^{-1}(p)\), \(\exists \mathcal{V}_q= V\) onde \(\pt J(F\circ G)(\textbf{x})=r\), ou seja que \(D\equiv 0\). Logo, \(y_i(\textbf{x}) = y_i(\textbf{x}^{(r)}, q^{(n-r)})\). 
}\vspace{0.2cm}}
\parbox[b]{0.17\linewidth}{  }
\end{tcolorbox}

\begin{tikzpicture}[>=to, shift={(10.15cm,-3.15cm)}, baseline={(current bounding box.center)}, remember picture, overlay]
...
\item[]\begin{tcolorbox}[colframe=gray, colback=white, boxrule = 0.8pt] 
  ...
  \tcblower
  \textcolor{gray}{
  Seja \(\iota: \textbf{x}^{(r)} \mapsto (\textbf{x}^{(r)}, q^{(n-r)})\). Use \(\blacksquare\) \hyperlink{thmFLI}{(\emph{FLI})} na função 
  \(F\circ G\circ \iota : \textbf{x}^{(r)} \mapsto (\textbf{x}^{(r)}, y_i(\textbf{x}^{(r)},q))\). Termine de estabelecer as vizinhanças e conclua. }
\end{tcolorbox}
\end{enumerate}
...
\centering
\begin{tikzpicture}[scale = 1, >=to]%, shift={(14.85cm,-0.9cm)}, baseline={(current bounding box.center)}, remember picture, overlay]
  % Puntos
  \coordinate (W) at (0,0); 
  % Espacio V
  \draw[thick]   (W) +(0.9,0.1) to[bend left= 25]  + (0.6,0.6);
...
\end{tikzpicture}
\\ 
\vspace{0.3cm} \small Figura 1: \(\blacksquare\) \hyperlink{thmPC}{(\emph{Posto Constante})}  
\end{figure}
\vspace{0.3cm}

\begin{corollary}%[Colorarios]
  Em geral temos, \(\blacksquare\) \hyperlink{thmTFI}{(\emph{Função Inversa})} \(\leftrightharpoons\) \(\blacksquare\) \hyperlink{thmFLI}{(\emph{FLI})} e \(\blacksquare\) \hyperlink{thmFLS}{(\emph{FLS})} \(\leftrightharpoons\) \(\blacksquare\) \hyperlink{thmPC}{(\emph{Posto Constante})}. Sendo \(F\in C^1(U,\R^m)\) e \(p\in U\) temos: 
  \begin{enumerate}[label = \roman*.]
    \item Se \(\pt JF(p) = r=\min \{n,m\}\), então \( \exists \mathcal{V}_p\) onde \(JF_r(\textbf{x}) \in \operatorname{GL}_r(\R)\). 
    \item Se \(F\) é localmente injetiva, então é imersão em \(V\ab\subset U\) denso em \(U\). \textcolor{gray}{\(\rightarrow \) Segue do \(\blacksquare\) \hyperlink{thmPC}{(\emph{Posto Constante})} e da semicontinuidade inferior}
    \item Se \(F\) é aberta, então é submersão em \(V\ab\subset U\) denso em \(U\). 
  \end{enumerate}
\end{corollary}

\alerta{
  \centering O posto deve ser constante em TODA uma vizinhança
  \tcblower
\begin{example}
  \(F: \R^2 \ni (t,s)\mapsto (t^2, t^3, s ) \in \R^3\) ¿Existem coordenadas como no \(\blacksquare\) \hyperlink{thmPC}{(\emph{Posto Constante})} nalguma vizinhança da origem? \textcolor{gray}{\(\rightarrow \) Não} %\(\operatorname{rank}(JF(0,0))\)}
\end{example}
}