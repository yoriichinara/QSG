...
\begin{definition}
    \(\phi \in \hom^k(V,W)\) é \(\textcolor{gray}{\rotatebox{90}{\Big|}}\star\textcolor{gray}{\rotatebox{90}{\Big|}}\) se, \(\forall (v_{i<j}) := (\cdots , v_i, \cdots, v_j, \cdots)\in \Pi^k V\),   
...
\begin{definition}
    Seja \(V\) \(\F\)-espaço vetorial. Uma $k$\emph{-ésima potência exterior} pra \(V\) é um par universal \((\phi, U)\) sobre \(X = \Pi^k V\) onde,   
...
\begin{corollary}
    \(\phi(\alpha^k_<) = \{\wedge v_{i_j} : i_j < i_{j+1}\}\) é base de \(\bigwedge^k V\). Em particular, se \(\dim V = n \), então \(\dim (\bigwedge^k V) = \binom{n}{k}\) e do \(\blacksquare\) \ref{thm:1043} \(\dim (A^k(V,W)) = \binom{n}{k} \dim W \). \comentario{Coisa de contar}
\end{corollary}

\begin{exercise}
    Se \((v_j) \in \Pi^k V\) é uma familia de vetores l.d., então \(\wedge v_j = 0\). Em particular, \(\dim A^k(V,W) = 0\) pra \(k >  n = \dim V\).  \comentario{Desarma usando $k$-linearidade} 
\end{exercise}
...
\begin{definition}
    Seja \(V\) \(\F\)-espaço vetorial. Uma $k$\emph{-ésima potência simétrica} pra \(V\) é um par universal \((\phi, U)\) sobre \(X = \Pi^k V\) onde,   
    \begin{itemize}[left = 1cm]
...
\begin{note}
    Notação não padrão pra a $k$-ésima potência simétrica, \((\phi, U) = (\odot , \odot^k V)\). De novo, as propiedades que à definem tem sua versão com este símbolo.  
\end{note}

\begin{corollary}
    \(\phi(\alpha^k_\leq) = \{\odot v_{i_j} : i_j \leq i_{j+1}\}\) é base de \(\bigwedge^k V\). Em particular, se \(\dim V = n \), então \(\dim (\odot ^k V) = \binom{n-1+k}{n-1}\) e do \(\blacksquare\) \ref{thm:1047} \(\dim (S^k(V,W)) = \binom{n-1+k}{n-1} \dim W \). \comentario{Conteo}
\end{corollary}
