...
\begin{definition}
    Seja \(f:U\ab\subseteq \R^n\to \R\). Dizemos que \(p \in U\) é extremo local da \(f\) se \(\exists \delta >0\) tal que se \(\|\x-p\|<\delta\), então \(f(p) \leq f(\x) \) ou \(f(\x) \leq f(p)\). 
\end{definition}
%\begin{exercise}
...
%\end{definition}
\begin{proposition}
    Se \(f\) é diff em \(p\) extremo local da \(f\), então \(p\) é ponto crítico. \textcolor{gray}{\(\rightarrow \ 0\) es extremo local de \(\varphi_i(t) = f(p+te_i)\)}
\end{proposition}
% \begin{note}
...
\begin{definition}
    \(A \in M_{n }(\R)\) é \emph{positiva} se \(\forall v\in \R^n \setminus \{0\}\), \( \phi (v,v) := [v]^t A [v] =  \langle v, Av\rangle > 0\), \emph{negativa} se \(\phi(v,v)<0\) ou \emph{indefinida} se for outro o caso. 
\end{definition}
...
\end{theorem}
\demo{Use \ \(\mathbb{S}^{n-1} \ni \vec{u}\mapsto \langle u, Hf(p)\cdot u \rangle\) pra ver que existe uma cota. Interprete essa cota no desenvolvimiento de Taylor de ordem $2$. }
...
\begin{theorem}[\emph{Bolzano-Weierstrass}]
    Sejam \(K\subseteq \R^n\) compacto e \(f:K\to \R\) continua. Então, \(f\) tem máximo e mínimo global em \(K\). 
\end{theorem}
%\begin{note}
%     
%\end{note}
...
\begin{definition}
    Seja  \(g\in C^k(U)\) tal que \(\forall p\in U : g(p)=0\), \(dg(p)\neq 0\). O conjunto \(^k H:= \{\textbf{x} \in \R^n : g(\textbf{x}) = 0\}\) é uma \emph{hipersuperficie} de classe \(C^k\). 
\end{definition}
