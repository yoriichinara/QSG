...
\begin{definition}
    Uma propiedade funcional \(P\) é \emph{compatível com composições} se \(f \circ g \mid P\) toda vez que \(\exists f \circ g\), \(f \mid P \) e \(g\mid P\). 
\end{definition}
...
\demo{
    ...
    Da universalidade \(\exists ! \widetilde{\psi}_a, \ \widetilde{\psi}_b\) como no diagrama. Veja que \(\widetilde{\psi}_a\) e \(\widetilde{\psi}_b\) são inversas a uma da otra mostrando primeiro que \((\widetilde{\psi}_a \circ \widetilde{\psi}_b) \circ \phi_a = \id_{_U} \circ \phi_a \) e \((\widetilde{\psi}_b \circ \widetilde{\psi}_a) \circ \phi_b = \id_{_V} \circ \phi_b \), conclua usando o  {\scriptsize \(\square\)} \hyperlink{lemma1012}{10.1.2.}. 
}
}
