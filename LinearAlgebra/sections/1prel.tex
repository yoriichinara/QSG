...
\begin{definition}
    Sejam \(T: V \to W\) linear, \(\alpha = (v_j)\) e \(\beta = (w_i)\) bases de \(V\) e \(W \) respetivamente. Então, se \([T(v_j)]_\beta= (a_{i_j})\), a \emph{matriz asociada} \([T]_\beta^\alpha := (a_{i_j})\) determina \(T\) no sentido que, \(\forall v \in V\), \([T(v)]_\beta = [T]_\beta^\alpha [v]_\alpha\). 
\end{definition}
...
\begin{theorem}
    \label{thm:6.1.6}
    Sejam \(\alpha = (v_i)\) base de \(V\) e \(\beta = (w_i)\) familia de vetores em \(W\), então \(\exists ! T:V\to W \) linear tal que \(\forall i \in I,\ T(v_i) = w_i\). \comentario{\cite[Pág. 189]{MA719}}
\end{theorem}
...
\begin{note}
    A transformação \(\zeta\) da \(\square\) \ref{prop:0.1} (b) é sobrejetiva se \(\#J < \infty\).
\end{note}
...
%\end{definition}
\begin{definition}
    Sejam \(\{t^i: i \in \mathbb{Z}_{\geq 0}\}\) conjunto respeitando as leis usuais de potências e \((\F,+,\cdot)\) um corpo.\footnote{Quer dizer que \(t^0 =1\) e \(t^m\cdot t^n=t^{m+n}\).  } Defina, \(\forall i \in \mathbb{Z}_{\geq 0}\), o espaço vetorial \(V_i = \{a_it^i: a_i \in \F, \  t^i \in I\}\), com as operações, 
    \begin{itemize}
